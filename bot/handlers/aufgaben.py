...
async def handle_loeschen(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Bestätigung für Löschen (de+en Ja-Token, s. synonyme.JA)
    ...
    parts = text.split()
    if len(parts) != 2 or parts[0] not in s.get("loeschen_tasks", {}) or parts[1] not in ("p", "x"):
    ...
    # wurde – ein zwischenzeitlich ERLEDIGTER (oder gelöschter/pausierter) Task
    # darf nicht mehr überschrieben werden, sonst verschwindet der Erledigt-
    # Eintrag samt Gefühl aus Statistik und Streak.
    _loeschbare_status = ("offen", "gefragt", "gefuehl_pending", "reaktion_pending", "serie_aktiv")
    ...
    if aktion == "p":
        ...
        tasks = await qdrant.get_tasks_by_status(["offen", "gefragt", "gefuehl_pending", "reaktion_pending", "serie_aktiv"])
        aufgabe_text = next(
            (t.get("aufgabe", "?") for t in tasks if t.get("qdrant_point_id") == point_id), "?"
        )
